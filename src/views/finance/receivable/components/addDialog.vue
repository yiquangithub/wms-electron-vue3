<template>
  ...
  >
    <div class="main-content">
      ...
      >
        <el-form-item label="收款金额" prop="actualAmount">
          <el-input
        ...
        <el-form-item label="收款日期" prop="actualDate">
          <el-date-picker
        ...
        <el-form-item label="付款方式" prop="payType">
          <el-input v-model="formData.payType" placeholder="请输入付款方式">
          </el-input>
        </el-form-item>
...
import { ref } from 'vue'
import { uploadReceivable, saveReceivable } from '@/api/finance/receivable'
...
const acceptParams = async (params) => {
  ...
  if (params.row.id) {
    await uploadReceivable(params.row.id).then((res) => {
...
const handleSubmit = async () => {
  ruleFormRef.value.validate(async (valid) => {
    if (valid) {
      let res = await saveReceivable(formData.value)
      if (res.code === 200) {

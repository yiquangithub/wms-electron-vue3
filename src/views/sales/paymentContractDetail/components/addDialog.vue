<template>
  ...
  >
    <div class="main-content">
      ...
      >
        <el-form-item label="回款阶段" prop="payPhase">
          <el-select v-model="formData.payPhase" placeholder="请选择回款阶段">
            <el-option
        ...
        <el-form-item label="回款比例" prop="percent">
          <el-input v-model="formData.percent" placeholder="请输入回款比例"
            ><template #append>%</template></el-input
          >
        </el-form-item>
        <el-form-item label="回款金额" prop="planAmount">
          <el-input v-model="formData.planAmount" placeholder="请输入回款金额">
...
import { ref } from "vue";
import { addPayment, updatePayment } from "@/api/sales/payment";
...
const rules = reactive({
  percent: [{ required: true, message: "请输入回款比例", trigger: "blur" }],
  planAmount: [{ required: true, message: "请输入回款金额", trigger: "blur" }],
  planDate: [{ required: true, message: "请选择日期", trigger: "change" }],
  payPhase: [{ required: true, message: "请选择回款阶段", trigger: "change" }],
});
const formData = ref({
  contractId: null,
  id: 0,

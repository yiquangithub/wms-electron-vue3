      ...
      >
        ...
        <div class="card-section">
          <div class="table-actions">
            <p
              v-if="dialogProps.isQuote"
              style="display: flex; align-items: center"
            >
              <span style="width: 120px">选择报价单：</span>
              <el-select v-model="countId" size="medium" @change="changRefList">
                <el-option
...
const acceptParams = async (params) => {
  ...
  if (params.isQuote) {
    formData.value.refMainId = params.row.id;
    // 获取报价次数
  ...
  if (params.row.id && !params.isQuote) {
...
const getMyQuotationDetail = async (id) => {
  ...
  await getQuotationDetail({ id: id }).then((res) => {
    const data = res.data;
    // 将询价单位信息赋值给供方
    formData.value.supplierId = data.inquiryUnitId; // 询价单位id - 供方id
    formData.value.supplierName = data.inquiryUnitName; // 询价单位名称- 供方名称
    formData.value.supplierContactsId = data.inquiryUnitContactsId; // 询价联系人- 供方单位联系人ID
    // 获取供方联系人列表
    if (formData.value.supplierId) {
      getContactUnitsListHandle(formData.value.supplierId);
    }
...
const handleDeleteMaterial = (row) => {
  const index = formData.value.meterials.findIndex(
    (item) => item.id === row.id
  );
  if (index > -1) {

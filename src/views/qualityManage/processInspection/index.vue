  ...
  <div class="test-template layout-padding">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="工序名称">
                <el-input
                  v-model="searchForm.workProcessName"
                  placeholder="请输入工序名称"
                />
              </el-form-item>
            </el-col>
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="工序编码">
                <el-input
                  v-model="searchForm.workProcessCode"
                  placeholder="请输入工序编码"
                />
              </el-form-item>
            </el-col>
    ...
    >
      ...
      >
        <el-row :gutter="20" class="mb20">
          ...
          <el-col :span="8">
            <el-form-item label="检验类型" prop="checkType">
              ...
              >
                <el-option
                  v-for="item in processTestTypeList"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                >
        ...
        <el-row :gutter="20" class="mb20">
          <el-col :span="8">
            <el-form-item label="工单编码" prop="workOrderId">
              <el-select
          ...
          <el-col :span="8">
            <el-form-item label="工序名称" prop="workProcessId">
              <el-select
                v-model="formData.workProcessId"
                placeholder="请选择工序名称"
                filterable
                @change="selectWorkProcessChange"
              >
                <el-option
      ...
      <div class="view-content">
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="工单编码">{{
            viewData.workOrderCode
          }}</el-descriptions-item>

          <el-descriptions-item label="工序编码">{{
            viewData.workProcessCode
          }}</el-descriptions-item>
          <el-descriptions-item label="工序名称">{{
            viewData.workProcessName
          }}</el-descriptions-item>
...
import {
  ...
  getProcessTestList,
  getProcessTestTypes,
  addOrEditProcessTest,
  deleteProcessTest,
  getProcessTestDetail,
} from "@/api/quality/processTest";

import { getProductDownList } from "@/api/leanScheduling/productManage";
import { getAllWorkOrders } from "@/api/leanScheduling/workOrder";
import { getAllWorkProcess } from "@/api/leanScheduling/workProcess";
...
const getTableData = async () => {
  ...
  try {
    const res = await getProcessTestList(params);
...
const dialogTitle = computed(() =>
  formData.value.id ? "编辑过程检验单" : "新增过程检验单"
);
const formRef = ref();
...
const handleAdd = async () => {
  ...
  // 自动获取编码
  try {
    const res = await autoProcessTestCode();
...
const handleEdit = async (row) => {
  const res = await getProcessTestDetail({ id: row.id });
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await addOrEditProcessTest(formData.value);
...
const handleDelete = async (row) => {
  try {
    ...
    if (confirm) {
      await deleteProcessTest({ id: row.id });
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    if (confirm) {
      await deleteProcessTest(selectedRows.value.map((item) => item.id));
      ElMessage.success("删除成功");
      getTableData(); // 重新获取数据
...
// processTestType
const processTestTypeList = ref([]);
const getProcessTestTypeList = async () => {
  const res = await getProcessTestTypes();
  processTestTypeList.value = res.data;
};
...
const getWorkProcessData = async () => {
  try {
    const res = await getAllWorkProcess({});
    workProcessList.value = res.data || [];
    // formData.value.workProcessName = res.data[0].name;
  } catch (error) {
    console.error("获取工序列表失败", error);
    workProcessList.value = [];
  }
};

const selectWorkProcessChange = (id) => {
  const item = workProcessList.value.find((item) => item.id == id);
  formData.value.workProcessName = item.name;
};

onMounted(() => {
  getTableData();
  getProcessTestTypeList();
});
</script>

...
import { getPurchaseMaterials } from '@/api/purchase/purchase.js'
...
const acceptParams = async (params) => {
  ...
  if (params.isBatch) {
    ...
    formData.value.purchaseId = params.purchaseId + ''
    const res = await getPurchaseMaterials(formData.value.purchaseId)
    console.log(res.data)
    formData.value.meterials = res.data.map((item) => ({
      id: item.id,
      ...
      purchasePrice: item.priceIncludeTax * item.purchaseQuantity, // 报价金额,
    }))
  }
  // 修改单个询价单

          ...
          <el-card class="record-card" :class="getCardClass(record)">
            ...
            <div class="record-content">
              <div class="record-auditor">
                <span class="auditor-label">申请人：</span>
              ...
              <div v-if="record.checkStatus === 1" class="approval-result">
                <div class="result-label">审核结果：</div>
                <div class="result-text" style="color: #409eff; font-weight: bold;">
                  审核通过
                </div>
              </div>
              <div v-else-if="record.checkStatus === 2" class="approval-result">
                <div class="result-label">审核结果：</div>
                <div class="result-text" style="color: #f56c6c; font-weight: bold;">
                  审核未通过
                </div>
              </div>
            </div>
...
const mockAuditData = [
  {
    ...
    checkStatus: 1,
    checkStatusStr: '已通过',
    checkRemark: '我是技术部前端开发李悦，2025年2月15日入职至今，试用期已满。现能独立完成需求对接与功能实现，融入团队节奏。申请转正，未来我会持续学习新技术，为项目高效交付贡献力量，盼领导批准。',
    checkResult: '审核通过',
    checkResultColor: '#409eff',
...
const acceptParams = async (params) => {
  dialogTitle.value = params.row.name + ' 转正申请审核记录'
  dialogVisible.value = true
  // 使用假数据
  auditRecords.value = mockAuditData
}
